...
### 權限請求流程 (2026-01-30 優化)

`QuickRecordScreen` 進入時會**先請求所有權限，再開始錄音**：
...
2. 請求位置權限
   - 拒絕 → Alert 詢問是否繼續（可選擇繼續錄音或返回）
   - 允許 → 繼續

3. 短暫延遲 (200ms) 讓 UI 穩定
...
**設計考量：**
- 麥克風權限是必要的，拒絕則無法使用
- 位置權限是可選的，拒絕後仍可錄音（稍後手動輸入地點）
- 所有權限在開始錄音前請求完畢，避免錄音過程中被打斷
...
## License Plate Input UX (2026-01-30 更新)

所有車牌輸入欄位都遵循統一的驗證規則：

### 輸入驗證
- 只接受大寫英文字母 (A-Z) 和數字 (0-9)
- 自動將小寫轉換為大寫
- 最大長度 8 字元
- 顯示提示：「-」可以不用輸入
...
| 頁面 | 檔案 |
|------|------|
| 登入（車牌登入） | `LoginScreen.tsx` |
| Onboarding | `LicensePlateScreen.tsx` |
| 發送流程 | `PlateInputScreenV2.tsx` |
| 一鍵語音 | `QuickVoiceSendScreen.tsx` |
| 儲存車牌 | `SavedPlatesScreen.tsx` |
| 車牌變更 | `LicensePlateChangeScreen.tsx` |

## Onboarding: 車牌已被註冊流程 (2026-01-30 更新)

當用戶在 Onboarding 輸入已被註冊的車牌時，會顯示三選項對話框：
...
│                                     │
│  車牌 ABC-1234 已有人註冊。         │
│  註冊方式：Apple 帳號               │  ← 顯示註冊方式幫助回想
│                                     │
│  ┌─────────────────────────────┐   │
│  │ 🔑 我忘記用哪個帳號註冊了   │   │ → 提示試其他登入方式
│  └─────────────────────────────┘   │
│                                     │
│  ┌─────────────────────────────┐   │
│  │ 🚗 這是我的車，但被別人註冊 │   │ → 進入車牌申請流程
│  └─────────────────────────────┘   │
│                                     │
│  ┌─────────────────────────────┐   │
│  │ ← 重新輸入車牌              │   │ → 清空輸入
...
### 隱私設計
- **不顯示**：綁定者的暱稱或手機號碼
- **只顯示**：註冊方式（Apple 帳號 / LINE 帳號）
- 這避免了透過猜車牌查詢他人資訊的隱私問題
...
interface LicensePlateCheckResponse {
  available: boolean;
  isBound: boolean;
  authProvider?: 'apple' | 'line';  // 註冊方式（不暴露個人資訊）
  hasPendingApplication?: boolean;
}
```

### Key Files
- `apps/api/src/users/users.service.ts` → `checkLicensePlateAvailability()`
- `apps/mobile/src/screens/onboarding/LicensePlateScreen.tsx` → 對話框 UI
- `apps/web/src/app/onboarding/page.tsx` → Web 版對話框
- `packages/shared/src/types/index.ts` → `LicensePlateCheckResponse`
