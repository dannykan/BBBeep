    ...
    FileInterceptor('file', {
      ...
      fileFilter: (req, file, callback) => {
        if (!ALLOWED_VOICE_TYPES.includes(file.mimetype)) {
          callback(new BadRequestException('只允許上傳語音檔案（M4A、MP3、WAV）'), false);
          return;
        }
        callback(null, true);
      },
  ...
  async transcribeVoice(@UploadedFile() file: Express.Multer.File) {
    ...
    try {
      ...
      const transcription = await this.openai.audio.transcriptions.create({
        ...
        prompt:
          '這是台灣人的繁體中文語音，關於行車或交通情況。常見詞彙：切過來、逼車、擦撞、違規、超車、併排、雙黃線、紅燈、綠燈、斑馬線、人行道、車道、迴轉、待轉、機車、汽車、計程車、公車、捷運、高鐵。請使用繁體中文轉錄。',
      });
      ...
      // 檢查是否為無法辨識的結果（Whisper 有時會回傳 prompt 內容）
      ...
      if (isUnrecognizable || text.trim().length === 0) {
        return {
          text: '',
          error: 'AI 無法辨識語音內容，請確保錄音清晰或改用文字輸入',
        };
      }
