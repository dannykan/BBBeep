/**
 * 不當言語詞庫
 * 包含髒話、威脅、騷擾、歧視等不當內容
 *
 * 2026-01 更新：現在只顯示警告，不阻擋發送，所以可以加入更多詞彙
 */

/**
 * 髒話/粗話（台灣常見）
 */
export const PROFANITY_WORDS = [
  // ===== 常見髒話 =====
  '幹', '幹你', '幹您', '干你', '干您',
  '靠', '靠北', '靠杯', '靠背', '靠么', '靠腰', '靠邀', '靠夭', '哭北', '哭夭', '尻',
  '媽的', '馬的', '他媽', '他馬', '你媽', '妳媽', '老母', '娘的',
  '操', '操你', '肏', '肏你', '草泥馬', '草你',
  '機掰', '雞掰', '機八', '雞八', '機巴', '雞巴', '基掰', '幾掰', '幾八', '基八', 'ㄐㄅ', '雞歪',
  '三小', '啥小', '殺小', '蝦小', '瞎小', '三洨', '啥洨', '沙小', '啥洨',

  // ===== 罵人笨/白目 =====
  '北七', '白癡', '白痴', '白目', '智障', '低能', '87', '八七', '白七',
  '蠢', '蠢蛋', '蠢貨', '笨蛋', '笨', '傻', '傻逼', '傻B', '腦殘', '沒腦', '無腦',
  '弱智', '腦子有洞', '腦袋裝屎', '豬腦', '豬頭', '呆子', '白爛',
  '二百五', '二貨', '憨', '憨狗', '憨包', '呆瓜',

  // ===== 罵人討厭/煩 =====
  '屁眼', '屁孩', '小屁孩', '死小孩', '死屁孩',
  '狗屎', '狗屁', '放屁', '屁話', '鬼話', '廢話',
  '廢物', '垃圾', '敗類', '人渣', '賤人', '賤貨', '賤', '下賤',
  '噁心', '噁', '嘔', '作嘔', '想吐',
  '討厭', '煩', '煩死', '吵死', '很煩', '好煩', '超煩',

  // ===== 叫人走開/去死 =====
  '去死', '快去死', '怎麼不去死', '去死吧', '死去',
  '閉嘴', '滾', '滾開', '滾蛋', '滾出去', '滾啦', '給我滾',
  '消失', '滾遠點', '別來煩', '走開',

  // ===== 罵人壞/不道德 =====
  '王八', '王八蛋', '龜兒子', '狗娘養', '雜種', '混蛋', '混帳',
  '畜生', '禽獸', '不是人', '沒人性', '沒良心',
  '可惡', '可恨', '該死', '死', '去死',

  // ===== 台語髒話（各種語音轉文字可能的寫法）=====
  '幹林', '幹林娘', '幹林老師', '幹林老母', '幹您娘', '幹妳娘', '幹你娘',
  '林北', '林杯', '恁爸', '恁北', '恁娘', '你爸', '你老師', '老師勒', '恁祖媽',
  '賽', '塞', 'ㄙㄞˋ', '屎', '大便', // 台語「屎」
  '膣', '膣屄', '雞歪', '機歪', // 台語粗話
  '夭壽', '夭壽喔', '夭壽仔', '妖壽', // 台語負面用語
  '死囝仔', '死查某', '死查埔', '臭查某', '臭查埔', // 台語罵人
  '靠爸', '靠母', '靠爸族', '靠勢',
  // 台語罵人常見詞
  '番', '番仔', '番顛', '起番', '番癲', // 發瘋、不講理
  '歹勢', '派', '派頭', // 有時候帶嘲諷
  '俗辣', '俗仔', '孬種', '沒卵', '沒卵蛋', '沒種', '沒膽', '膽小鬼', '縮', '縮頭',
  '龜', '龜毛', '龜縮', '縮起來', '龜孫', '龜孫子',
  '豬哥', '色鬼', '色狼', '鹹豬手', '吃豆腐',
  '歹命', '衰', '衰小', '衰鬼', '帶賽', '觸霉頭', '烏鴉嘴',
  '顧人怨', '討人厭', '白爛', '爛', '爛人', '爛貨', '爛咖',
  '垃圾', '垃圾車', '破爛', '破銅爛鐵',
  '盧', '盧小', '盧洨', '歡', '歡北北', // 很煩、一直吵
  '機車', '很機車', '超機車', '機車人', // 台灣特有用法
  '白爛', '很白爛', '白目', '白目仔',
  '假仙', '假掰', '裝模作樣', '裝', '裝死', '裝傻', '裝蒜',
  '雞婆', '管很多', '愛管', '多管閒事',
  '奧客', '澳洲來的', // 難搞的客人
  '落漆', '漏氣', '漏屎', '漏尿', // 失敗、丟臉
  '觸', '觸小', // 倒楣
  '哭爸', '哭母', '哭夭', '靠夭', '哭哭', // 抱怨
  '塞', '塞車', '塞住', '塞爆',
  '抓狂', '起肖', '發瘋', '瘋了', '瘋子', '起笑',
  '搞屁', '搞毛', '搞什麼', '在搞',
  '三八', '38', '三八阿花', // 形容女生傻
  '肖查某', '肖仔', '神經', '神經質',
  '哈囉', '是在哈囉', '哈', '蛤', '蛤?',
  '厚', '厚話', '厚臉皮', '臉皮厚',
  '痞', '痞子', '流氓', '混混', '8+9', '八加九', '八嘎囧',
  '草莓族', '媽寶', '爸寶',
  '阿達', '阿答', '秀逗', '腦袋秀逗', // 腦子有問題
  '瞎', '瞎咧', '瞎爆', '瞎透',
  '扯', '太扯', '扯爆', '扯淡', '鬼扯',
  '誇張', '很誇張', '太誇張',
  '離譜', '很離譜', '太離譜', '超離譜',
  '傻眼', '傻眼貓咪', '無言', '無語',
  '莫名其妙', '無理取鬧', '沒道理',

  // ===== 質疑性/攻擊性語句 =====
  '會不會開車', '不會開車', '眼睛長哪', '眼睛呢', '有沒有眼睛',
  '瞎了', '眼瞎', '瞎了眼', '眼睛瞎',
  '是在哈囉', '哈囉', '有事嗎', '腦子有問題', '有毛病',
  '搞什麼', '搞屁', '搞毛', '在搞', '你在幹嘛',
  '什麼東西', '什麼玩意', '啥玩意',

  // ===== 嘲諷/挑釁 =====
  '呵呵', '嘻嘻', '嘿嘿', '哈哈哈哈哈', // 嘲諷語氣
  '厲害喔', '好棒喔', '好厲害', '神經病', '有病', '腦子壞了',
  '笑死', '笑屎', '笑死人', '可笑', '好笑',
  '活該', '自找的', '自作自受', '報應',

  // ===== 性相關不當用語 =====
  '雞巴', '屌', '懶叫', '懶覺', 'lj', '老二', '小弟弟',
  '奶子', '奶', '胸', // 在不當語境
  '騷', '騷貨', '淫', '淫蕩', '色狼', '變態', '色胚',
  '婊', '婊子', '妓女', '援交', '賣', '破麻',

  // ===== 諧音/變體/注音 =====
  '㊣', 'ㄍㄢˋ', 'ㄇㄉ', 'ㄐㄅ', 'ㄎㄅ', 'ㄙㄢˇㄒㄧㄠˇ', 'ㄅㄑ',
  'gan', 'gann', 'fxxk', 'fk', 'wtf', 'stfu', 'fck', 'fuc', 'fuk',
  'fxck', 'fook', 'fack', 'faq', 'fvck',
  'fk', 'fku', 'fu', 'fuu',
  'fml', 'omfg', 'stfu', 'gtfo', 'kys',
  'fkyou', 'fku', 'fkoff',
  'fxxx', 'fxk', 'ffk',
  'fku', 'fu2', 'fuu',
  'fml', 'omfg', 'stfu', 'gtfo', 'kys',
  'shit', 'sht', 'shiit', 'shitt', 'bullshit', 'bs',
  'damn', 'damm', 'dammit', 'damnit',
  'ass', 'arse', 'asshole', 'a-hole',
  'bitch', 'btch', 'biatch',
  'dick', 'dck', 'd1ck',
  'crap', 'crp',
  'hell', 'heck',
  'idiot', 'moron', 'dumb', 'stupid',

  // ===== 網路用語/縮寫 =====
  '吃屎', '吃大便', '吃💩',
  '腦X', '腦殘', '智X',
  'gg', 'GG', // 有時候帶嘲諷
  '可撥', '可悲',
  '嗆', '嗆屁', '嗆什麼',
  '酸', '酸民', '酸什麼',
  '噴', '噴子', '亂噴',
  '黑', '黑人', // 惡意批評
  '崩潰', '氣死', '氣炸',
];

/**
 * 威脅性言語
 */
export const THREAT_WORDS = [
  // ===== 暴力威脅 =====
  '殺', '殺了你', '殺死', '砍', '砍死', '弄死', '打死', '整死',
  '揍', '揍你', '揍死', '打你', '打爆', '扁你', '扁死', '修理你',
  '踹', '踹死', '踢死', '踢爆',
  '撞', '撞死', '撞爆', '輾', '輾過',

  // ===== 威脅語句 =====
  '等著瞧', '等著看', '有你好看', '好看', '走著瞧',
  '小心點', '給我小心', '放尊重', '給我放', '最好',
  '找人', '找兄弟', '找人收拾', '叫人', '叫兄弟',
  '報復', '後果', '後果自負', '自己負責', '別怪我',
  '威脅', '恐嚇', '讓你', '讓你知道', '給你好看',
  '你完了', '完蛋了', '死定了', '等死', '玩完了',
  '小心你的車', '小心車子', '記住你', '記住你的車',
  '知道你住哪', '找到你', '讓你好看',

  // ===== 詛咒 =====
  '詛咒', '下地獄', '不得好死', '斷子絕孫',
  '出門被車撞', '全家', '全家死', '你全家',
];

/**
 * 騷擾性言語
 */
export const HARASSMENT_WORDS = [
  // ===== 搭訕/性騷擾 =====
  '約炮', '約砲', '一夜', '一夜情',
  '帥哥', '美女', '妹子', '小姐姐', '正妹', '辣妹',
  '想認識', '交個朋友', '聊聊', '私聊', '密我',
  '單身', '有男友', '有女友', '有對象',
  '幾歲', '住哪', '在哪', '哪裡人',
  '電話', '手機', '號碼', '加我', '加好友', 'line', 'ig',

  // ===== 跟蹤相關 =====
  '跟蹤', '偷拍', '偷看', '監視',
  '知道你家', '找到你家', '等你下班',
];

/**
 * 歧視性言語
 */
export const DISCRIMINATION_WORDS = [
  // ===== 性別歧視 =====
  '女司機', '女人開車', '母豬', '台女', '綠茶', '綠茶婊',
  '娘炮', '娘娘腔', '人妖', '死gay', '死同',

  // ===== 地域歧視 =====
  '426', '阿陸', '阿六', '支那', '強國人',
  '南部人', '鄉下人', '台巴子',

  // ===== 身心障礙歧視 =====
  '殘廢', '瞎子', '聾子', '啞巴', '瘸子', '跛腳',
  '精神病', '神經病', '瘋子', '瘋婆',

  // ===== 年齡歧視 =====
  '老人家', '老頭', '老太婆', '老不死', '老廢物',
  '三寶', '馬路三寶',

  // ===== 其他歧視 =====
  '外勞', '外傭', // 應使用「移工」「外籍看護」
  '窮鬼', '乞丐', '魯蛇', 'loser',
];

/**
 * 所有不當言語合併（用於警告顯示）
 * 2026-01 更新：納入騷擾和歧視詞，因為現在只是警告不阻擋
 */
export const ALL_PROFANITY = [
  ...PROFANITY_WORDS,
  ...THREAT_WORDS,
  ...HARASSMENT_WORDS,
  ...DISCRIMINATION_WORDS,
];

/**
 * 高嚴重度詞彙（顯示強烈警告）
 */
export const HIGH_SEVERITY_WORDS = [
  ...THREAT_WORDS,
  // 極端髒話
  '幹你娘', '幹您娘', '操你媽', '肏你媽', '幹林娘',
  '去死', '快去死', '怎麼不去死', '去死吧',
  // 嚴重歧視
  '支那', '母豬',
];

/**
 * 中嚴重度詞彙
 */
export const MEDIUM_SEVERITY_WORDS = [
  '幹', '靠', '靠北', '靠杯', '媽的', '機掰', '雞掰',
  '白癡', '白目', '智障', '廢物', '垃圾', '腦殘',
  '滾', '閉嘴', '噁心',
];

/**
 * 檢測是否包含高嚴重度詞彙
 */
export function hasHighSeverityWord(text: string): string | null {
  const lowerText = text.toLowerCase();
  for (const word of HIGH_SEVERITY_WORDS) {
    if (lowerText.includes(word.toLowerCase())) {
      return word;
    }
  }
  return null;
}

/**
 * 檢測是否包含任何不當言語
 * @returns 匹配到的詞彙，若無則返回 null
 */
export function findProfanity(text: string): string | null {
  const lowerText = text.toLowerCase();
  for (const word of ALL_PROFANITY) {
    if (lowerText.includes(word.toLowerCase())) {
      return word;
    }
  }
  return null;
}

/**
 * 取得詞彙的嚴重程度
 */
export function getProfanitySeverity(word: string): 'low' | 'medium' | 'high' {
  const lowerWord = word.toLowerCase();

  if (HIGH_SEVERITY_WORDS.some(w => lowerWord.includes(w.toLowerCase()))) {
    return 'high';
  }

  if (MEDIUM_SEVERITY_WORDS.some(w => lowerWord.includes(w.toLowerCase()))) {
    return 'medium';
  }

  return 'low';
}
