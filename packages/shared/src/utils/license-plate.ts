...
import { LICENSE_PLATE } from '../constants';
...
export function normalizeLicensePlate(plate: string): string | null {
  ...
  if (
    formatted.length < LICENSE_PLATE.MIN_LENGTH ||
    formatted.length > LICENSE_PLATE.MAX_LENGTH
  ) {
    return null;
  }
...
export function isValidLicensePlate(plate: string): boolean {
  return normalizeLicensePlate(plate) !== null;
}
